...
function draw() {
    ...
    if (mouseX >= leftSide && mouseX <= rightSide && mouseY >= topSide && mouseY <= bottomSide) {      //  if mouse is within bounds to manipulate
        if (samplerButton.state == 'play' || samplerButton.state == 'stop') {   //  if audio has been loaded
            ...
            if (mapMouseX < 0) {    //  switch to backwards if mouse is on negative side of x-axis
                samplerButton.playerForward.volume.rampTo(-100, 0.01);
                samplerButton.playerBackward.volume.rampTo(0, 0.01);
            }
            else {  //  switch to forwards if mouse is on positive side of x-axis
                samplerButton.playerForward.volume.rampTo(0, 0.01);
                samplerButton.playerBackward.volume.rampTo(-100, 0.01);   
            }
